...
class ProductRepository:
    ...
    def get_products_with_low_stock(self) -> list[Product]:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT * FROM products WHERE quantity <= 10').fetchall()
            return [Product(**dict(r)) for r in rows]
    ...
    def get_product_with_highest_price(self) -> Optional[Product]:
        with self.db.connect() as conn:
            row = conn.execute('SELECT * FROM products ORDER BY price DESC LIMIT 1').fetchone()
            return Product(**dict(row)) if row else None

    def get_product_with_lowest_price(self) -> Optional[Product]:
        with self.db.connect() as conn:
            row = conn.execute('SELECT * FROM products ORDER BY price ASC LIMIT 1').fetchone()
            return Product(**dict(row)) if row else None
