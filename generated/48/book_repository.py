...
class BookRepository:
    ...
    def list(self, title: Optional[Any] = None, author: Optional[Any] = None, genre: Optional[Any] = None, publication_year: Optional[Any] = None, publication_year_end: Optional[Any] = None, isbn: Optional[Any] = None, pages: Optional[Any] = None, max_pages: Optional[Any] = None, min_pages: Optional[Any] = None) -> List[Book]:
        with self.db.connect() as conn:
    ...
    def get_books_by_isbn_prefix(self, prefix: str) -> list[Book]:
        return []
    ...
    def get_books_with_most_pages(self) -> list[Book]:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT * FROM books ORDER BY pages DESC LIMIT 10').fetchall()
            return [Book(**dict(r)) for r in rows]

    def get_books_by_author_and_genre(self, author: str, genre: str) -> list[Book]:
        return self.list(
            author=author,
            genre=genre,
        )

    def get_books_with_pages_between(self, min_pages: int, max_pages: int) -> list[Book]:
        return self.list(
            min_pages=min_pages,
            max_pages=max_pages,
        )

    def get_books_with_author_or_genre_match(self, author: str, genre: str) -> list[Book]:
        return self.list(
            author=author,
            genre=genre,
        )

    def get_books_published_after(self, year: int) -> list[Book]:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT * FROM books WHERE publication_year > ?', (year,)).fetchall()
            return [Book(**dict(r)) for r in rows]


    def get_books_by_author_like(self, *args, **kwargs):
        return self.get_books_by_author(*args, **kwargs)


    def get_books_by_genre_like(self, *args, **kwargs):
        return self.get_books_by_genre(*args, **kwargs)
