...
class ProjectService:
    ...
    def search_tasks_by_title(self, query: str, project_id: int) -> List[Dict[str, Any]]:
        """
            Search tasks by title within a specific project using the task repository.
        ...
        task_list = []
        for task in results:
            task_dict = {'id': task.id, 'title': task.title, 'description': task.description, 'priority': task.priority, 'status': task.status, 'project_id': task.project_id, 'created_at': task.created_at, 'updated_at': task.updated_at}
    ...
    def get_total_tasks_by_status(self) -> Dict[str, int]:
        """Retrieve the total count of tasks by status (e.g., 'pending', 'in_progress', 'completed')."""
        status_counts = {}
        statuses = ['pending', 'in_progress', 'completed']
        for status in statuses:
            tasks = self.task_repo.get_tasks_by_status_and_priority_with_project_info(status, None)
            count = sum((1 for task in tasks if task.get('status') == status))
