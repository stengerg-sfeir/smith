...
class TaskRepository:
    ...
    def get_tasks_with_due_date_range(self, start_date: str, end_date: str) -> list[Task]:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM tasks WHERE due_date BETWEEN ? AND ?', (start_date, end_date))
    ...
    def get_overdue_tasks(self) -> list[Task]:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM tasks WHERE due_date < datetime('now', 'start of day')")
            rows = cursor.fetchall()
            return [Task(**dict(r)) for r in rows]
    ...
    def get_tasks_with_high_priority_and_due_soon(self) -> list[Task]:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM tasks WHERE priority = 'high' AND due_date >= datetime('now', 'start of day') AND due_date < datetime('now', '+1 day')")
            rows = cursor.fetchall()
            return [Task(**dict(r)) for r in rows]
