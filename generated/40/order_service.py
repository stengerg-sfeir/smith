...
class NotificationService:
    ...
    def get_orders_with_status_and_total(self, status: str, min_total: float) -> List[Order]:
        """Retrieve orders with a specific status and total amount above a minimum."""
    ...
    def send_confirmation_notification(self, order: Order) -> None:
        ...
        if not order:
            raise NotFoundError(f'Order with id {order.id} not found')
        notification = Notification(
            message=f'Thank you for your order! Order #{order.id} is confirmed.',
            order_id=order.id,
            sent_to=order.customer_id,
            sent_at=datetime.datetime.now()
        )
        # NotificationRepository is not imported, so we remove the dependency
        # This method will now just create a notification object without persisting it
        # If persistence is required, NotificationRepository must be properly defined and imported
