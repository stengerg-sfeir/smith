...
class BookRepository:
    ...
    def get_total_available_copies(self) -> int:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT available_copies FROM books WHERE available_copies > 0').fetchall()
    ...
    def get_books_by_year_range(self, start_year: int, end_year: int) -> List[Book]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM books WHERE published_year >= ? AND published_year <= ?", ((start_year, end_year))
            ).fetchall()
            return [Book(**dict(r)) for r in rows]
