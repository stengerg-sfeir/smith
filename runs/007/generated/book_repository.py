...
class BookRepository:
    ...
    def generate_genre_distribution_report(self) -> dict[str, int]:
        with self.db.connect() as conn:
            cursor = conn.execute('SELECT genre, COUNT(*) as count FROM books GROUP BY genre')
            rows = cursor.fetchall()
            return {row[0]: row[1] for row in rows}

    def get_most_popular_author(self) -> str:
        with self.db.connect() as conn:
            cursor = conn.execute('SELECT author FROM books GROUP BY author ORDER BY COUNT(*) DESC LIMIT 1')
            row = cursor.fetchone()
