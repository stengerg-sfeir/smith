...
class ProductRepository:
    ...
    def get_product_report_by_category(self) -> dict[str, dict]:
        with self.db.connect() as conn:
            cursor = conn.execute('SELECT category, COUNT(*) as product_count, SUM(price * quantity) as total_value FROM products GROUP BY category')
            ...
            for row in rows:
                ...
                product_count = row[1]
                total_value = row[2]
                report[category] = {'product_count': product_count, 'total_value': total_value}
            return report

    def get_product_summary(self) -> dict:
        with self.db.connect() as conn:
            cursor = conn.execute('SELECT COUNT(*) as total_products, SUM(price * quantity) as total_value, AVG(price) as average_price FROM products')
            row = cursor.fetchone()
            return {'total_products': row[0] if row else 0, 'total_value': row[1] if row else 0, 'average_price': row[2] if row else 0}
