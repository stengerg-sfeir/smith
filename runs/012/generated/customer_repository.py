...
class CustomerRepository:
    ...
    def get_customer_count_by_domain(self, domain: str) -> int:
        with self.db.connect() as conn:
            cursor = conn.execute('SELECT COUNT(*) FROM customers WHERE email LIKE ?', (f'%@{domain}',))
            row = cursor.fetchone()
    ...
    def get_customers_with_valid_email_format(self) -> list[Customer]:
        with self.db.connect() as conn:
            cursor = conn.execute("SELECT email, id, name, phone FROM customers WHERE email LIKE '%@%' AND email LIKE '%@%.%'")
    ...
    def get_customers_by_name_and_domain(self, name: str, domain: str) -> list[Customer]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM customers WHERE (name LIKE ? OR email LIKE ? OR phone LIKE ?) AND name = ?",
