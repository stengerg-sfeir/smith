...
class UserRepository:
    ...
    def get_user_with_most_orders(self) -> User:
        with self.db.connect() as conn:
            ...
            if not rows:
                return User(id=None, email=None, password_hash=None, role=None)
            row = rows[0]
    ...
    def get_active_products_count(self) -> int:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT COUNT(*) FROM products WHERE stock_quantity > 0').fetchall()
    ...
    def get_total_orders_by_status(self, status: str) -> int:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT COUNT(*) FROM orders WHERE status = ?', (status,)).fetchall()
            return rows[0][0] if rows else 0
