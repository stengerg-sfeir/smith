...
class ProductRepository:
    ...
    def bulk_update_stock(self, product_ids: list[int], new_stock_quantity: int) -> bool:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            try:
                cursor.execute('UPDATE products SET stock_quantity = ? WHERE id IN ({})'.format(','.join(('?' for _ in product_ids))), [*product_ids, new_stock_quantity])
    ...
    def get_products_with_low_stock_alert(self) -> list[Product]:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT id, name, category, description, price, stock_quantity, created_at, updated_at FROM products WHERE stock_quantity <= 10')
            rows = cursor.fetchall()
            return [Product(**dict(r)) for r in rows]
