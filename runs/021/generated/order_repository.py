...
class OrderRepository:
    ...
    def get_orders_with_customer_name_filter(self, customer_name: str) -> list[Order]:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM orders WHERE customer_id IN (SELECT id FROM customers WHERE name = ?)', (customer_name,))
            rows = cursor.fetchall()
            return [Order(**dict(r)) for r in rows]

    def get_order_count_by_status_and_date_range(self, status: str, start_date: datetime, end_date: datetime) -> dict[str, int]:
        return self.list(
            status=status,
        )
    ...
    def get_active_orders_count_by_customer_id(self, customer_id: int) -> int:
        with self.db.connect() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM orders WHERE customer_id = ? AND status = 'active'", (customer_id,))
        ...
        with self.db.connect() as conn:
            ...
            for row in rows:
                ...
                if month not in trend_data:
                    trend_data[month] = []
                trend_data[month].append(status)
            return trend_data
