...
class MemberRepository:
    ...
    def get_members_by_name_prefix(self, prefix: str) -> list[Member]:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT * FROM members WHERE name LIKE ?', (f'{prefix}%',)).fetchall()
            return [Member(**dict(r)) for r in rows]
    ...
    def get_members_with_loans_overdue(self) -> list[Member]:
        with self.db.connect() as conn:
            rows = conn.execute("\n                SELECT DISTINCT m.*\n                FROM members m\n                JOIN loans l ON m.id = l.member_id\n                WHERE l.due_date < datetime('now')\n                AND l.status = 'active'\n            ").fetchall()
            return [Member(**dict(r)) for r in rows]

    def get_total_books_borrowed_by_member(self, member_id: int) -> int:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT COUNT(*) FROM loans WHERE member_id = ?', (member_id,)).fetchone()
            return rows[0] if rows else 0

    def get_members_by_email_domain(self, domain: str) -> list[Member]:
        with self.db.connect() as conn:
            rows = conn.execute('SELECT * FROM members WHERE email LIKE ?', (f'%@{domain}',)).fetchall()
            return [Member(**dict(r)) for r in rows]
