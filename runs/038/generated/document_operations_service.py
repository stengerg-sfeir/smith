...
class AuthService:
    ...
    def get_user_documents(self, user_id: int, title_filter: Optional[str]=None, created_after: Optional[datetime.datetime]=None, created_before: Optional[datetime.datetime]=None) -> List[Document]:
        """
            Retrieve documents for a user with optional filtering by title, creation date range, and status.
            """
    ...
    def check_document_access(self, user_id: int, document_id: int) -> bool:
        ...
        if not document:
            raise NotFoundError(f'Document with id {document_id} not found for user {user_id}')
        permissions = self.user_repo.get_user_document_permissions(user_id=user_id)
