...
class CustomerRepository:
    ...
    def list(self, first_name: Optional[Any] = None, last_name: Optional[Any] = None, email: Optional[Any] = None, phone: Optional[Any] = None) -> List[Customer]:
        with self.db.connect() as conn:
    ...
    def get_customers_by_last_name_prefix(self, last_name_prefix: str, page_number: int, page_size: int) -> list[Customer] & dict[str, int]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM customers WHERE last_name LIKE ? LIMIT ? OFFSET ?",
                ("%" + last_name_prefix + "%", page_size, (page_number - 1) * page_size)
            ).fetchall()
            return [Customer(**dict(r)) for r in rows]
    ...
    def get_customers_with_email_domain(self, domain: str, page_number: int, page_size: int) -> list[Customer] & dict[str, int]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM customers WHERE (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ?) LIMIT ? OFFSET ?",
                ("%" + domain + "%", "%" + domain + "%", "%" + domain + "%", "%" + domain + "%", page_size, (page_number - 1) * page_size)
            ).fetchall()
            return [Customer(**dict(r)) for r in rows]
    ...
    def get_customers_by_phone_pattern(self, phone_pattern: str, page_number: int, page_size: int) -> list[Customer] & dict[str, int]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM customers WHERE phone LIKE ? LIMIT ? OFFSET ?",
                ("%" + phone_pattern + "%", page_size, (page_number - 1) * page_size)
            ).fetchall()
            return [Customer(**dict(r)) for r in rows]
    ...
    def get_customers_with_recent_activity(self, days_ago: int, page_number: int, page_size: int) -> list[Customer] & dict[str, int]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT * FROM customers WHERE updated_at >= datetime('now', '-' || ? || ' days') LIMIT ? OFFSET ?",
                (days_ago, page_size, (page_number - 1) * page_size)
            ).fetchall()
            return [Customer(**dict(r)) for r in rows]
