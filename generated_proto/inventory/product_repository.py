...
class ProductRepository:
    ...
    def list_by_category(self, category_id: Optional[int] = None) -> list[Product]:
        return self.list(
    ...
    def aggregate_stock_value_by_category(self) -> dict[str, int]:
        with self.db.connect() as conn:
            rows = conn.execute(
                "SELECT o.name AS k, SUM(e.price_cents * e.stock_qty) AS v FROM products e JOIN categories o ON e.category_id = o.id GROUP BY o.name",
            ).fetchall()
